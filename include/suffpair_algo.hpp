...
void compute_lms_as_sp(gram_info_t & gram,
                       pairing_data& p_data,
                       size_t lpg_syms){
    bv_t new_r_lim;
...
void get_rep_suff_rules(i_file_stream<size_t>& rules, pairing_data& p_data, bv_t& uniq_nr){
    //mark the new suffix phrases with one occurrence
...
void prepare_input(gram_info_t& gram_info, bv_t& rep_syms, sdsl::cache_config& config){
    ...
    ivb rules(gram_info.rules_file, std::ios::in);
...
void merge_ht_data(std::vector<thread_data>& t_data){
    ...
    for(auto & data : t_data){
        ...
        bitstream<ht_t::buff_t> bits;
        bits.stream = reinterpret_cast<ht_t::buff_t*>(k_buffer);

...
int main (int argc, char** argv){
    ...
    dbuff_i_file_stream<uint8_t, 1024> db_if(input_file);
    auto start = high_resolution_clock::now();
    size_t sym_freq[256]={0};
    for(size_t i=0;i<db_if.size();i++){
        sym_freq[db_if.value()]++;
        db_if.next();
    }
    auto end = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(end - start);
    std::cout<<"mt: "<<duration.count()<<" microseconds"<<std::endl;

    std::cout<<"Next approach"<<std::endl;
    i_file_stream<uint8_t> ifs(input_file, page_size*1024);
    start = high_resolution_clock::now();
    size_t sym_freq2[256]={0};
    for(size_t i=0;i<ifs.size();i++){
        sym_freq2[ifs.read(i)]++;
    }
    end = high_resolution_clock::now();
    duration = duration_cast<microseconds>(end - start);
    std::cout<<"st: "<<duration.count()<<" microseconds"<<std::endl;

    for(size_t i=0;i<256;i++){
        if(sym_freq[i]!=0){
            assert(sym_freq[i]==sym_freq2[i]);
        }
    }

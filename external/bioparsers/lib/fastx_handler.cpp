...
hp_collection fastx2plain_format(const std::string& input_file, std::string const &output_file) {
...
void get_DNA_rc(std::string const& r_heads_file, std::string const& r_len_file, str_collection& str_coll){
    std::ifstream ifs;
    std::ofstream ofs;
    ...
    while(true){
        ...
        for(size_t j=read_bytes, i=0;j-->0;i++){
            sym = in_h_buffer[j];
            if(sym<85){
                out_h_buffer[i] = (char)dna_string::rev_comp[sym];
            }else{
                out_h_buffer[i] = (char) (dna_string::rev_comp[sym-84]+84);
            }
        }
    ...
    for(size_t i=0;i<str_coll.alphabet.size();i++){
        ...
        rc_sym = dna_string::rev_comp[sym];
        if(sym>=85) rc_sym =dna_string::rev_comp[sym-84]+84;

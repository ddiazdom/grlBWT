...
//extract freq symbols from bwt[j] onwards and put them in new_bwt
                ...
                if (sym == dict.hocc_dummy) {// copy from the buffer of induced symbols
                    ...
                    while (pbwt_freq > 0) {
                        ...
                        if (sym == dict.bwt_dummy) {//from the bwt i+1
                            ...
                            insert_ssr(freq, ssr_pos_prev_bwt, next_lim_prev_bwt, consumed_syms_prev_bwt, new_bwt_size,
                                       is_ssr_end_prev_bwt, ssr_list_prev_bwt);
                            extract_rl_syms(bwt_buff, new_bwt_buff, j, freq);
                            new_bwt_size += freq;
                    ...
                    //std::cout<<"Pre BWT"<<std::endl;
                    insert_ssr(pbwt_freq, ssr_pos_prev_bwt, next_lim_prev_bwt, consumed_syms_prev_bwt, new_bwt_size,
                               is_ssr_end_prev_bwt, ssr_list_prev_bwt);
                    extract_rl_syms(bwt_buff, new_bwt_buff, j, pbwt_freq);
                    new_bwt_size += pbwt_freq;
    ...
    template<uint8_t b_f_r>
    //b_f_r = number of bytes to encode the length of a BWT run

...
void insert_ssr(size_t freq, size_t& ssr_pos, size_t& next_lim, size_t& consumed_syms, size_t new_bwt_size, bool& is_ssr_end,
                std::vector<std::pair<size_t, size_t>>& old_ssr_list/*, std::vector<std::pair<size_t, size_t>>& new_ssr_list*/){

    while(ssr_pos<old_ssr_list.size() && next_lim < (consumed_syms+freq)) {
        if(is_ssr_end){
            //std::cout<<"end: "<<new_bwt_size + (next_lim-consumed_syms) <<" consumed syms "<<consumed_syms<<" freq "<<freq<<" new_bwt_size: "<<(new_bwt_size+freq)<<" next lim "<<next_lim<<"\n"<<std::endl;
            old_ssr_list[ssr_pos].second = new_bwt_size + (next_lim-consumed_syms);
            next_lim = old_ssr_list[++ssr_pos].first;
        }else{
            //std::cout<<"start: "<<new_bwt_size + (next_lim-consumed_syms) <<" consumed syms "<<consumed_syms<<" freq "<<freq<<" new_bwt_size: "<<(new_bwt_size+freq)<<" next lim "<<next_lim<<std::endl;
            old_ssr_list[ssr_pos].first = new_bwt_size + (next_lim-consumed_syms);
            //new_ssr_list.emplace_back(new_bwt_size + (next_lim-consumed_syms), 0);
            next_lim = old_ssr_list[ssr_pos].second;
        }
        is_ssr_end = !is_ssr_end;
    }
}

//extract freq symbols from bwt[j] onwards and put them in new_bwt
void opt_extract_rl_syms(bwt_buff_writer& bwt_buff, bwt_buff_writer& new_bwt_buff, size_t& j, size_t freq){
...
size_t opt_compute_hocc_size(dictionary& dict, bv_rs_t& hocc_rs, vector_t& hocc_buckets,
...
void opt_infer_lvl_bwt(tmp_workspace& ws, size_t p_round) {
    ...
    size_t n_runs = opt_compute_hocc_size(dict, hocc_rs, hocc_buckets, p_round, ws);
            ...
            if(sym==dict.hocc_dummy) {// copy from the buffer of induced symbols
                ...
                while(pbwt_freq>0) {
                    ...
                    if(sym==dict.bwt_dummy) {//from the bwt i+1
                        opt_extract_rl_syms(bwt_buff, new_bwt_buff, j, freq);
                    }else{
                        if(new_bwt_buff.size()>0 && new_bwt_buff.last_sym()==sym){
                ...
                opt_extract_rl_syms(bwt_buff, new_bwt_buff, j, pbwt_freq);
...
void opt_infer_lvl_bwt(tmp_workspace& ws, size_t p_round) {
    ...
    //get the ranges for the suffixes
    bool is_ssr_end_prev_bwt = false;
    size_t consumed_syms_prev_bwt=0, ssr_pos_prev_bwt=0, next_lim_prev_bwt;
    std::vector<std::pair<size_t, size_t>> ssr_list_prev_bwt;
    load_pl_vector(ws.get_file("same_suffix_"+std::to_string(p_round+1)), ssr_list_prev_bwt);
    next_lim_prev_bwt = ssr_list_prev_bwt[ssr_pos_prev_bwt].first;
        ...
        if(sym>=dict.bwt_dummy) {// an unsolved segment of the preliminary BWT
            ...
            if(sym==dict.hocc_dummy) {// copy from the buffer of induced symbols
                ...
                while(pbwt_freq>0) {
                    ...
                    sym--;
                    if(sym==dict.bwt_dummy) {//from the bwt i+1
                        //std::cout<<"Pre BWT"<<std::endl;
                        insert_ssr(freq, ssr_pos_prev_bwt, next_lim_prev_bwt, consumed_syms_prev_bwt, new_bwt_size, is_ssr_end_prev_bwt, ssr_list_prev_bwt);
                        opt_extract_rl_syms(bwt_buff, new_bwt_buff, j, freq);
                        new_bwt_size+=freq;
                        consumed_syms_prev_bwt+=freq;
                    }else{
                        if(new_bwt_buff.size()>0 && new_bwt_buff.last_sym()==sym){
                            new_bwt_buff.inc_freq_last(freq);
                        } else {
                ...
                //std::cout<<"Pre BWT"<<std::endl;
                insert_ssr(pbwt_freq, ssr_pos_prev_bwt, next_lim_prev_bwt, consumed_syms_prev_bwt, new_bwt_size, is_ssr_end_prev_bwt, ssr_list_prev_bwt);
                opt_extract_rl_syms(bwt_buff, new_bwt_buff, j, pbwt_freq);
                new_bwt_size+=pbwt_freq;
                consumed_syms_prev_bwt+=pbwt_freq;
            }
        }else{// a segment in the preliminary BWT that was already solved
            if(new_bwt_buff.size()>0 && new_bwt_buff.last_sym()==sym){
                new_bwt_buff.inc_freq_last(pbwt_freq);
            ...
            //std::cout<<"Curr BWT"<<std::endl;
            //insert_ssr(pbwt_freq, ssr_pos_curr_bwt, next_lim_curr_bwt, new_bwt_size, new_bwt_size, is_ssr_end_curr_bwt, ssr_list_curr_bwt/*, new_ssr_list*/);
            new_bwt_size+=pbwt_freq;
        }
        i++;
    }
    ...
    std::vector<std::pair<size_t, size_t>> ssr_list_curr_bwt;
    load_pl_vector(ws.get_file("same_suffix_"+std::to_string(p_round)), ssr_list_curr_bwt);

    std::vector<std::pair<size_t, size_t>> new_ssr_list;
    new_ssr_list.reserve(ssr_list_prev_bwt.size()+ssr_list_curr_bwt.size());

    while(pos_a<ssr_list_prev_bwt.size() && pos_b <ssr_list_curr_bwt.size()){
        if(ssr_list_prev_bwt[pos_a].first<ssr_list_curr_bwt[pos_b].first){
            new_ssr_list.push_back(ssr_list_prev_bwt[pos_a++]);
        }else{
            new_ssr_list.push_back(ssr_list_curr_bwt[pos_b++]);
        }
    }
    while(pos_a<ssr_list_prev_bwt.size()){
        new_ssr_list.push_back(ssr_list_prev_bwt[pos_a++]);
    }
    while(pos_b<ssr_list_curr_bwt.size()){
        new_ssr_list.push_back(ssr_list_curr_bwt[pos_b++]);
    }

    //TODO testing the information is correct
    assert(new_ssr_list.size()==new_ssr_list.capacity());
    for(i=0;i<new_ssr_list.size()-1;i++){
        //std::cout<<new_ssr_list[i].first<<" "<<new_ssr_list[i].second<<std::endl;
        //std::cout<<new_ssr_list[i+1].first<<" "<<new_ssr_list[i+1].second<<"\n"<<std::endl;
        assert(new_ssr_list[i].first<=new_ssr_list[i].second);
        assert(new_ssr_list[i].second<new_ssr_list[i+1].first);
    }
    //
...
void opt_ind_phase(tmp_workspace& ws, size_t p_round){
    ...
    while(p_round-->0){
        ...
        if constexpr (b_f_r==0){
            opt_infer_lvl_bwt(ws, p_round);
        }else{
            opt_infer_lvl_bwt<b_f_r>(ws, p_round);
        }
        auto end = std::chrono::steady_clock::now();
...
template void opt_ind_phase<0>(tmp_workspace& ws, size_t p_round);
template void opt_ind_phase<1>(tmp_workspace& ws, size_t p_round);
template void opt_ind_phase<2>(tmp_workspace& ws, size_t p_round);
template void opt_ind_phase<3>(tmp_workspace& ws, size_t p_round);
template void opt_ind_phase<4>(tmp_workspace& ws, size_t p_round);
template void opt_ind_phase<5>(tmp_workspace& ws, size_t p_round);

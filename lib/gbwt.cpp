...
void infer_lvl_bwt_v2(sdsl::cache_config& config, size_t p_round) {
    ...
    //TODO new version
    std::string pbwt_file = sdsl::cache_file_name("pre_bwt_lvl_"+std::to_string(p_round), config);
    bwt_buff_reader p_bwt(pbwt_file);
    assert(p_bwt.size()==(pre_bwt.size()/2));
    //
    ...
    while(i<pre_bwt.size()) {
        ...
        size_t tmp_sym;
        size_t tmp_freq;
        p_bwt.read(i/2, tmp_sym, tmp_freq);
        assert(sym == tmp_sym);
        assert(pbwt_freq == tmp_freq);
        //

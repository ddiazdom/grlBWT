...
size_t create_new_rules(phrase_map_t &ht, gram_info_t &p_gram, bv_t &rep_sym,
                        sdsl::cache_config &config, vector_t &rules, bv_t &r_lim) {
...
void compress_grammar(gram_info_t& p_gram, vector_t& rules, bv_t& r_lim, bv_t& rep_sym, phrase_map_t& ht, sdsl::cache_config &config) {
    ...
    size_t n_new_rules = create_new_rules(ht, p_gram, rep_sym, config, rules, r_lim);
    ...
    size_t prev_g_size = p_gram.g, new_g_size=new_pos;
    ...
    //remove_uniq_nt(rep_sym, rules, r_lim, p_gram);

    std::cout<<"    Stats:"<<std::endl;
    std::cout<<"      Grammar size before:        "<<prev_g_size<< std::endl;
    std::cout<<"      Grammar size after:         "<<new_g_size<<std::endl;
    std::cout<<"      Number of new nonterminals: "<<n_new_rules<<std::endl;
    std::cout<<"      Compression ratio:          "<<double(new_g_size)/double(prev_g_size) << std::endl;
...
void mark_repeated(bv_t& rep_sym, vector_t& rules, bv_t& r_lim, gram_info_t& p_gram){
    ...
    size_t nt=p_gram.max_tsym+1, sym;
...
void suffpair(gram_info_t& p_gram, sdsl::cache_config &config) {
    ...
    //mark the repeated symbols
    bv_t rep_sym(p_gram.r, false);
    mark_repeated(rep_sym, rules, r_lim, p_gram);
    ...
    //hash the suffixes that might be repeated
    j=end;
